...
echo ::::::::::::::::::::::::
echo ::INJECTIINE [TurboCD]::
echo ::::::::::::::::::::::::
SLEEP 3

:: CHECK THAT FILES EXIST

IF NOT EXIST *.iso GOTO:404ISOnotFound
IF NOT EXIST *.cue GOTO:404CUEnotFound
...
:EnterKeyLOT
cls
set BASEID=0005000010167b00
set BASEPDC=PN5E
set BASEFOLDER="LORDS OF THUNDER [PN5EJ8]"

IF EXIST TitleKeyLOT.txt goto:EnterCommon
echo This step will not be required the next time you start Injectiine.
echo Enter the title key for Lords of Thunder (USA):
set /p TITLEKEY=
echo %TITLEKEY:~0,32%>TitleKeyLOT.txt
set /p TITLEKEY=<TitleKeyLOT.txt
cls
IF "%TITLEKEY:~0,4%"=="9d87" GOTO:EnterCommon ELSE GOTO:WrongKeyLOT

:LOT
set BASEID=0005000010167b00
set BASEPDC=PN5E
set BASEFOLDER="LORDS OF THUNDER [PN5EJ8]"
IF EXIST TitleKeyLOT.txt (set /p TITLEKEY=<TitleKeyLOT.txt) ELSE (GOTO :EnterKeyLOT)
GOTO:EnterCommon
...
echo Injectiine will now create an TurboCD injection.
...
IF EXIST bootSound.btsnd (move bootSound.btsnd ../Tools/CONSOLES/TurboCD/WORKDIR/meta/bootSound.btsnd)

cd ..
move Files\*.iso Tools\CONSOLES\TurboCD\Tools
move Files\*.cue Tools\CONSOLES\TurboCD\Tools
cd Tools
cd CONSOLES
cd TurboCD
cd Tools
...
:: Rename CUE
2>NUL ren *.cue CDROM.cue

:: Update CUE and rename ISO
for %%f in (*.iso) do (
	fart -i CDROM.cue "%%f" CDROM.iso
	2>NUL ren "%%f" CDROM.iso
)

:: Extract ISO
bincuesplit CDROM.cue hcd

:: Delete the CDROM.iso and CDROM.cue
del CDROM.iso
del CDROM.cue
...
:: Rename ISO tracks to BIN
for %%X in (*.iso) do (
	..\fart CDROM.hcd "%%X" "%%~nX.bin"
	2>NUL ren "%%X" "%%~nX.bin"
)

:: Resample OGG files
echo Begin resampling audio files to 32000Hz
for %%f in (*.ogg) do (
	echo Resample "%%f"
	ren "%%f" resample
	..\ffmpeg.exe -hide_banner -loglevel error -i resample -qscale:a 5 -ar 32000 "%%f"
	del resample
)
echo Finished resampling audio files

cd ..

:: Create pce.pkg
BuildTurboCdPcePkg CDROM

:: Remove CDROM directory
rd /s /q CDROM
...
echo   ^<olv_accesskey type="unsignedInt" length="4"^>3375469152^</olv_accesskey^>>>meta.xml
...
move iconTex.tga ..\CONSOLES\TurboCD\WORKDIR\meta
move bootTvTex.tga ..\CONSOLES\TurboCD\WORKDIR\meta
move bootDrcTex.tga ..\CONSOLES\TurboCD\WORKDIR\meta
2>NUL move bootLogoTex.tga ..\CONSOLES\TurboCD\WORKDIR\meta
cls
...
cd ../CONSOLES/TurboCD
cd ..
move TurboCD\WORKDIR ..\..\Output\"[TurboCD] %GAMENAME% [%PRODUCTCODE%]"
...
cd ../CONSOLES/TurboCD
cd ..
move TurboCD\WORKDIR TurboCD\NUSPacker\WORKDIR
cd TurboCD
cd NUSPacker
java -jar NUSPacker.jar -in WORKDIR -out "[TurboCD] %GAMENAME% (000500001337%TITLEID%)"
rd /s /q tmp
rd /s /q WORKDIR
rd /s /q output
move "[TurboCD] %GAMENAME% (000500001337%TITLEID%)" ..\..\..\..\Output

:: Final check if game exists
:FinalCheck
cd ..\..\..\..\Output
IF NOT EXIST "[TurboCD] %GAMENAME% (000500001337%TITLEID%)" GOTO:GameError
GOTO:GameComplete

:FinalCheckLoadiine
cd ..\..\Output
IF NOT EXIST "[TurboCD] %GAMENAME% [%PRODUCTCODE%]" GOTO:LoadiineError
GOTO:GameComplete
...
IF /i "%PACKDECIDE%"=="y" echo "[TurboCD] %GAMENAME% (000500001337%TITLEID%)"
IF /i "%PACKDECIDE%"=="n" echo "[TurboCD] %GAMENAME% [%PRODUCTCODE%]"
